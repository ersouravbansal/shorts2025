...
async function fetchMoreVideos(pageNumber, videos, language) {
  try {
    ...
    const response1 = await axios.get(
      `${api_url}${BASEPATH}/api/videos/video/?pageNumber=${
        pageNumber || 1
      }&language=${language}`
    );
...
export const meta: MetaFunction = ({ data, params }) => {
  ...
  if (currentVideo) {
    ...
    const currentVideoUrl = `${baseurl}/videos/${currentVideoTitle}-${currentVideo.id}`;
...
const SingleVideoComponent = () => {
  ...
  async function loadMore() {
    setLoading(true);
    try {
      // console.log("calling load more",page,language,videos);
  ...
  console.log("videos from 'videos route' is:", videos);
  return (
    <>
      <Content
